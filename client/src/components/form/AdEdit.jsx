...
export function AdEditForm() {
  ...
  const { publicAds, adminRefreshAds } = useContext(AdsContext);
  ...
  useEffect(handleResetClick, [adId, publicAds]);

  function handleResetClick() {
    const adData = adId ? publicAds.filter((a) => a.id === +adId)[0] : null;

    if (adData) {
      setImg(adData.thumbnail);
      setName(adData.name);
      setPrice(adData.price);
      setDescription(adData.description);
      setCategory(adData.category_name);
    }
  }
